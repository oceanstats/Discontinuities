...
load("msv_results.Rdata")
weatherhead_betap <- total_betap  
...
disc3_betap2_weatherhead <- array(NA,dim=c(40000,23))

colnames(ResultsTab_no) <- c("Region","Trend Value","Lower Confidence Interval","Upper confidence interval","RMSE","discmag1","ldm1","udm1","discmag2","ldm2","udm2","discmag3","ldm3","udm3")
colnames(ResultsTab_weatherhead) <- c("Region","Trend Value","Lower Confidence Interval","Upper confidence interval","RMSE","discmag1","ldm1","udm1","discmag2","ldm2","udm2","discmag3","ldm3","udm3")
...
###test if significant difference between discontinuity magnitude and 0
if((bounds_weatherhead[17,1]>0 & bounds_weatherhead[17,2]>0) | (bounds_weatherhead[17,1]< 0 & bounds_weatherhead[17,2]< 0))
{
  isdiffdm3[indio] <- T
  }else{
  isdiffdm3[indio] <- F
...
disc3 <- mean(weatherhead_betap[17,,j],na.rm=T)
#RMSE
...
disc2_betap2_weatherhead[,indio]  <- weatherhead_betap[16,,j]
  ...
  disc3_betap2_weatherhead[,indio]  <- weatherhead_betap[17,,j]
     temp <- hdi(disc3_betap2_weatherhead[,indio] ,credMass=0.95)
  bounds_disc3 <- c(as.numeric(temp[1]),as.numeric(temp[2]))#HDI
...
#table of results
ResultsTab_no[indio,] <- c(indio,mean(no_betap[2,,j]),bounds_no[2,],RMSE_no,disc,bounds_disc,disc2,bounds_disc2,disc3,bounds_disc3)
ResultsTab_weatherhead[indio,] <- c(indio,mean(weatherhead_betap[2,,j]),bounds_weatherhead[2,],RMSE_weatherhead,disc,bounds_disc,disc2,bounds_disc2,disc3,bounds_disc3)
...
save(regave_weatherhead,regave_no,ResultsTab_no,ResultsTab_weatherhead,disc_betap2_weatherhead,disc2_betap2_weatherhead,disc3_betap2_weatherhead,Chlcontent,trend_betap2_no,trend_betap2_weatherhead,file="msv_Results_tables.Rdata")

     save(isdiff,isdiffdm1,isdiffdm2,isdiffdm3,file="msv_significance.Rdata")