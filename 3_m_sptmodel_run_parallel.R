# run parallel processing of sptimer under the MERIS/MODIS discontinuity scenario
user_wd='...' #user should specify the location of their working dircetory containing Longhurst_180.mat and BGC_input files
...
savename <- paste0(j,"_m_BGC_input.Rdata")
load(savename)
...
Model<- spT.Gibbs(
  ...
  savename <- paste0(j,"_m_BGC_burn.Rdata")
...
if(i>1)
{
  savename <- paste0(j,"_m_BGC_model_pt",i-1,".Rdata")#load last iteration
load(savename)
...
Model<- spT.Gibbs(
  ...
  savename <- paste0(j,"_m_BGC_model_pt",i-1,".Rdata")
