# run parallel processing of sptimer under the SeaWiFS discontinuity scenario
...
savename <- paste0(j,"_sw_BGC_input.Rdata")
load(savename)
...
Model<- spT.Gibbs(
  ...
  savename <- paste0(j,"_sw_BGC_burn.Rdata")
...
if(i>1)
{
  savename <- paste0(j,"_sw_BGC_model_pt",i-1,".Rdata")#load last iteration
load(savename)
...
Model<- spT.Gibbs(
  ...
  savename <- paste0(j,"_sw_BGC_model_pt",i-1,".Rdata")
