#file to combine separate chains from previous iteration for mv_scenario
user_wd='...'
library(R.matlab)
...
for(j in 46)
{
  ...
  for(i in 1:40) #stitching together multiple runs
  {
     ...
     savename <- paste0(user_wd,j,"_BGC_model_mv_pt",i,".Rdata")
  load(savename)
...
fittedChl[locind[m,1],locind[m,2],k] <- fittemp[m]
    ...
    savename <- paste0(user_wd,j,"_mv_trace.png")
    png(savename)
...
save(total_betap,model_input,fittedChl,file=paste0(user_wd,"mv_results.Rdata")
