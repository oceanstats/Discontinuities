# run parallel processing of sptimer under the VIIRS & MERIS/MODIS discontinuity scenario
...
savename <- paste0(j,"_mv_BGC_input.Rdata")
load(savename)
...
Model<- spT.Gibbs(
  ...
  savename <- paste0(j,"_mv_BGC_burn.Rdata")
...
if(i>1)
{
  savename <- paste0(j,"_mv_BGC_model_pt",i-1,".Rdata")#load last iteration
load(savename)
...
Model<- spT.Gibbs(
  ...
  savename <- paste0(j,"_mv_BGC_model_pt",i-1,".Rdata")
