...
load("no_results.Rdata")
no_betap <- total_betap

no_fittedChl <- fittedChl
Longhurst_n <- Longhurst
load("m_results.Rdata")
...
ResultsTab_no <- array(NA,dim=c(23,8))
ResultsTab_weatherhead <- array(NA,dim=c(23,10))
Chlcontent <- array(NA,dim=c(23))
isdiff<- array(NA,dim=c(23))
isdiffdm1<- array(NA,dim=c(23))
diffbetween=array(NA,dim=c(232,23))
...
colnames(ResultsTab_no) <- c("Region","Trend Value","Lower Confidence Interval","Upper confidence interval","RMSE","discmag1","ldm1","udm1")
colnames(ResultsTab_weatherhead) <- c("Region","Trend Value","Lower Confidence Interval","Upper confidence interval","RMSE","discmag1","ldm1","udm1","month5","month6")
...
disc <- mean(weatherhead_betap[15,,j])
montha <- mean(weatherhead_betap[7,,j])
monthb <- mean(weatherhead_betap[8,,j])
#RMSE
...
#table of results
ResultsTab_no[indio,] <- c(indio,mean(no_betap[2,,j]),bounds_no[2,],RMSE_no,disc,bounds_disc)
ResultsTab_weatherhead[indio,] <- c(indio,mean(weatherhead_betap[2,,j]),bounds_weatherhead[2,],RMSE_weatherhead,disc,bounds_disc,montha,monthb)
...
save(diffbetween,ResultsTab_no,ResultsTab_weatherhead,disc_betap2_weatherhead,Chlcontent,trend_betap2_no,trend_betap2_weatherhead,file="m_Results_tables.Rdata")

     save(isdiff,isdiffdm1,file="m_significance.Rdata")