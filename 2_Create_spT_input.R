...
sitetemp <-rep(1:(chl.dim[1]*chl.dim[2]),each=232)  # site index
#TT=rep(((time-mean(time))/sd(time)), times=(chl.dim[1]*chl.dim[2])) #use normalised time
TT=rep(time, times=(chl.dim[1]*chl.dim[2])) 
monthtemp <-as.factor(rep(c(9:12,rep(1:12,times=19)),times=(chl.dim[1]*chl.dim[2])))
...
temp <- rep(temp[,2],each=232)
spTmodel$site[temp==0] <- NA
spTmodel <- spTmodel[!is.na(spTmodel$site),]
spTmodel$site <- rep(1:(nrow(spTmodel)/length(time)),each=232) #replace site list with valid sites
